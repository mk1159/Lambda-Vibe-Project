...
export function appReducer(state: AppState, action: DispatchAction): AppState {
  ...
  const newState = (() => {
    switch (type) {
      ...
      case 'ADD_SONGS': {
        const current = state.get('songs');
        const newSong = args.get('new_song');
        const songTitle = newSong.get('title');
        const songNotes = newSong.get('notes');

        return state.set('songs',
          current.push(Map<string, any>({
            'id': current.size + 1,
            'songTitle': songTitle,
            'notes': songNotes
          }))
